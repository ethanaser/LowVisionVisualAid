...
public class WifiUtils {
    ...
    // 打开WIFI
    public boolean OpenWifiandScan() {
        boolean bRet = true;
        if (!mWifiManager.isWifiEnabled()) {
    ...
    private void deleteWifiConnect() {
        ...
        if (existingConfigs != null) {
            for (WifiConfiguration existingConfig : existingConfigs) {
                Log.d("ThinGlassesActivity", "deleteWifiConnect: " + existingConfig.SSID);
                forgetWifi(existingConfig.SSID);
    ...
    public boolean connectNet(String SSID, String Password, String Type) {
        ...
        deleteWifiConnect();//连接时要先设定其他wifi不可用
        forgetWifi(SSID);
     ...
     * @return
     */
    public String getFailReason(boolean isConnected) {
