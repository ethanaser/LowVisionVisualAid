...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
        Log.d(TAG, "requestPermission: " + device.getVendorId() + " productID  " + device.getProductId());
    ...
    private final void processDettach(final UsbDevice device) {
        if (destroyed) return;
        if (DEBUG)
            Log.v(TAG, "processDettach:" + device.getProductId() + " listener=null=" + (mOnDeviceConnectListener == null) + " handler=null=" + (mAsyncHandler == null));
        if (mOnDeviceConnectListener != null) {
