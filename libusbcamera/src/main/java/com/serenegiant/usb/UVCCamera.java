...
public class UVCCamera {
    ...
    public static final int PIXEL_FORMAT_YUV420P = 6;
...
//================================================================================
    ...
    public synchronized void setZoom(final int zoom) {
        if (mNativePtr != 0) {
            ...
            Log.d(TAG, "setZoom:mZoomMax=" + mZoomMax + " ,mZoomMin=" + mZoomMin);
            if (range > 0) {
    ...
    private int panMax=2000,panMin=-2000;

    public synchronized void setPan(int panValue){
        if (mNativePtr != 0) {
//            nativeSetPan(mNativePtr, 2000);
            final float range = Math.abs(panMax - panMin);
            Log.d(TAG, "setPan:mPanMax=" + panMax + " ,panMin=" + panMin);
            if (range > 0) {
                final int z = (int) (panValue / 100.f * range) + panMin;
                Log.d(TAG, "setPan:zoom=" + panValue + " ,value=" + z);
                nativeSetPan(mNativePtr, z);
            }
        }
    }

    private int tiltMax=900,tiltMin=-900;
    public synchronized void setTilt(int tiltValue){
        if (mNativePtr != 0) {
            final float range = Math.abs(tiltMax - tiltMin);
            Log.d(TAG, "setTilt:tiltMax=" + tiltMax + " ,tiltMin=" + tiltMin);
            if (range > 0) {
                final int z = (int) (tiltValue / 100.f * range) + tiltMin;
                Log.d(TAG, "setTilt:zoom=" + tiltValue + " ,value=" + z);
    ...
    public synchronized void setBackLight(final int backLight) {
        if (mNativePtr != 0) {
            ...
            if (range > 0) {
                final int z = (int) (backLight / 100.f * range) + mBacklightCompMin;
                nativeSetBacklightComp(mNativePtr, z);
            }
        }
    }

    public synchronized int getBackLight(final int backLight) {
        int result = 0;
        if (mNativePtr != 0) {
